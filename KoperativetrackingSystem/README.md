...
| Database connection error | Make sure PostgreSQL is running and the password in `application.yml` matches |
| Port 8080 already in use | Stop any other app using port 8080, or change the port in `application.yml` |
| Port 5173 already in use | Stop any other dev server, or run `npx vite --port 3000` |
| Login fails | Make sure the backend started successfully (check for errors in the terminal) |
